defmodule Rivulet.Kafka.Router do
  ...
  defmacro __before_compile__(env) do
    ...
    quote do
      def start_link do
        Rivulet.Kafka.Router.Funcs.start_link(
          __MODULE__,
          unquote(consumer_group),
          unquote(stream_topics)
        )
      end
      ...
      Macro.postwalk(routes, fn
        ...
        {:publish_to, _, [topic, config]} ->
          ...
          unless partition_strategy in @partition_strategies do
            raise "#{__CALLER__.module} publishing to #{topic} has an unsupported partition strategy: #{
                    partition_strategy
                  }"
          end
    ...
    case Module.get_attribute(module, :streams) do
      ...
      streams when is_list(streams) ->
        ...
        case dups do
          ...
          duplicates ->
            raise "The following stream topics were defined multiple times in #{module}: #{
                    inspect(duplicates)
                  }"
        end
